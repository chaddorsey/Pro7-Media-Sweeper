...
from Pro7_File_API_Python import propresenter_pb2  # Used to decode PlayList files, which do not have an extension
from Pro7_File_API_Python import propDocument_pb2  # Used to decode Props configuration file, which does not have an extension
from Pro7_File_API_Python import proworkspace_pb2  # Used to decode Workspace configuration file, which contains Mask info
from Pro7_File_API_Python import stage_pb2  # Used to decode Stage configuration file.
from Pro7_File_API_Python import playlistTemplate_pb2  # Used to decode Playlist Template files, which do not have an extension
...
def get_refs_in_file(file_obj, path, log_file):
    ...
    octal_unicode_regex = rb'((?<!\\)\\[0-7]{3}|(?<=\\\\)\\[0-7]{3})'
...
def pick_media_folder():
    open_file = filedialog.askopenfilename(initialdir=path_entry.get())  # Returns opened path as str
    if open_file != "":  # if folder dialog was not cancelled
        open_file = Path(open_file)
        path_entry.delete(0, tk.END)
        path_entry.insert(0, open_file.__str__())
        showinfo(
        title='Selected File',
        message=open_file
    )
...
def sweep_the_file():
    ...
    # Set file locations
    sweep_file_location = Path.expanduser(Path(path_entry.get()))
    ...
    if not os.path.exists(sweep_file_location):
        msg = sweep_file_location.__str__() + "\ndoes not exist!"
    ...
    write_file_line(log_file,
                    ...
                    "Chosen file to sweep:      " + sweep_file_location.__str__() + "\n" +
    ...
    ##### REMOVE – relevant only for folder sweep

    # TODO: This is not very efficient if include subdirectories is not checked.
    #media_files = []
    #for subdir, dirs, files in os.walk(sweep_file_location):
    #    for filename in files:
    #        status_label.config(text="Found Media file: " + filename)
    #        status_label.update()
    #        if cb.get() == 0 and subdir.upper == sweep_file_location.__str__().upper:
    ...
    #for filename in files:
    #if sweep_file_location.upper().endswith(".PRO"):
    if str(sweep_file_location).split(".")[-1] == "pro":
        showinfo(message="Presentation!")
        pro7_file_obj = presentation_pb2.Presentation()
        #filepath = Path(subdir) / Path(filename)
        filepath = sweep_file_location
    ...
    for move_file_from in files_to_move:
        start = len(sweep_file_location.__str__()) + 1
    ...
    if db.get() == 1:
        remove_empty_directories(sweep_file_location)
...
img_path = base_path / 'resource_files/icons/gifs/Sweeper64.gif'
